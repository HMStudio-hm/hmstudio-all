...
// HMStudio Combined Features (Quick View v2.3.3 & Announcement Bar v1.2.9)
...
;(() => {
  ...
  // Global QuickViewStats object
  window.QuickViewStats = {
  ...
  const QuickView = {
    ...
    async fetchProductData(productId) {
      const url = `https://europe-west3-hmstudio-85f42.cloudfunctions.net/getProductData?storeId=${this.config.storeId}&productId=${productId}`
    ...
    createVariantsSection(productData) {
      ...
      if (productData.variants && productData.variants.length > 0) {
        ...
        variantAttributes.forEach((attr) => {
          ...
          select.addEventListener("change", () => {
            this.updateSelectedVariant(productData)
          })
    ...
    createQuantitySelector(currentLang) {
    ...
    async handleAddToCart(productData) {
      const currentLang = getCurrentLanguage()
      const form = document.getElementById("product-form")
      ...
      if (productData.variants && productData.variants.length > 0) {
        ...
        if (!selectedVariant) {
          const message =
            currentLang === "ar" ? "هذا المنتج غير متوفر بالمواصفات المختارة" : "This product variant is not available"
          alert(message)
          return
        }
      ...
      try {
        // Assuming zid is a global object or imported elsewhere.
        ...
        zid.store.cart
          ...
          .then(async (response) => {
            if (response.status === "success") {
              try {
                await window.QuickViewStats.trackEvent("cart_add", {
    ...
    async displayQuickViewModal(productData) {
      const currentLang = getCurrentLanguage()

      try {
        await window.QuickViewStats.trackEvent("modal_open", {
      ...
      if (productData.images && productData.images.length > 0) {
        const gallery = this.createImageGallery(productData.images)
      ...
      if (productData.variants && productData.variants.length > 0) {
        const variantsSection = this.createVariantsSection(productData)
      ...
      const quantitySelector = this.createQuantitySelector(currentLang)
      ...
      addToCartBtn.addEventListener("mouseover", () => {
        addToCartBtn.style.backgroundColor = "#047857"
      })
  ...
  const AnnouncementBar = {
    async fetchAnnouncementSettings() {
      try {
    ...
    async initialize() {
      const settings = await this.fetchAnnouncementSettings()
