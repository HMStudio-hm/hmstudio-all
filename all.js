...
const quickViewObserver = new MutationObserver(() => {
  ...
  console.log('QuickView MutationObserver set up');
...
const announcementObserver = new MutationObserver((mutations) => {
  ...
  // ==========================================================
    ...
    // Start paste from the line: (function() {
    // End paste before the last line: })();
    // Remove the IIFE wrapper and adjust the script to use the shared getStoreIdFromUrl function
    // ==========================================
  
    // Sliding Cart Feature
    // ==========================================
    // PASTE SLIDING CART SCRIPT HERE
    // Start paste from the line: (function() {
    // End paste before the last line: })();
    // Remove the IIFE wrapper when pasting
    // ==========================================
  
    // Upsell Feature
    // ==========================================
    // PASTE UPSELL SCRIPT HERE
    // Start paste from the line: (function() {
    // End paste before the last line: })();
    // Remove the IIFE wrapper when pasting
    // ==========================================
...
async function initializeFeatures() {
    try {
      // Initialize Quick View if available
      if (typeof QuickView !== 'undefined') {
        QuickView.initialize();
      }
  
      // Initialize Announcement Bar if available
      if (typeof AnnouncementBar !== 'undefined') {
        AnnouncementBar.initialize();
      }
  
      // Initialize Smart Cart if available
      if (typeof SmartCart !== 'undefined') {
        SmartCart.initialize();
      }
  
      // Initialize Sliding Cart if available
      if (typeof SlidingCart !== 'undefined') {
        SlidingCart.initialize();
      }
  
      // Initialize Upsell if available
    ...
    function handleStoreEvents() {
      // Listen for Zid's cart events
      document.addEventListener('zid-add-to-cart', function(event) {
        if (typeof window.add_to_cart_event === 'function') {
          window.add_to_cart_event(event.detail);
        }
      });
  
      document.addEventListener('zid-purchase-completed', function(event) {
        if (typeof window.purchase_event === 'function') {
          window.purchase_event(event.detail);
        }
      });
    }
